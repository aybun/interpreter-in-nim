...
proc peekError(t : token.TokenType): void =
    var msg = &"expected next token to be {t}, got {p.peektoToken.Type} instead"
    p.errors.add(msg)
...
proc parseStatement(p: Parser) ast.Statement =
    case p.curToken.Type:
        ...
        of token.RETURN:
            return p.parseRetuernStatement()
        else:
...
proc peekPrecedence(p: Parser): Precedence =
    ...
    var p = precedences.getOrDefault(p.peekToken.Type, nil)`
    if p != nil:
        return p
    else:
        return LOWEST
...
proc curPrecedence(p: Parser): Precedence =
    ...
    var p = precedences.getOrDefault(p.curToken.Type, nil)
...
proc parseIntegerLiteral(p: Parser): ast.Expression =
    ...
    try:
        integer = parseInt(p.curToken.Literal)
    except ValueError:
...
proc parseBlockStatement(p: Parser): ast.BlockStatement = 
    ...
    var b = ast.BlockStatement(Token: p.curToken,
     Statements : newSeq[ast.Statements]())

    p.nextToken()

    while !p.curTokenIs(token.RBRACE) and !p.curTokenIs(token.EOF):
...
proc parseFunctionParameters(p: Parser): seq[Identifers] =
...
proc parseCallExpression(p: Parser, function : ast.Expression): ast.Exression =
    var ast.CallExpression(Token: p.curToken, Function: function)
...
func registerInfix(p: Parser, tokenType: token.TokenType, fn: inixParseFn): void =
