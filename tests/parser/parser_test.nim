...
suite "TestLetStatments":
  test "test":
    ...
    for i in 0..<len(tests):
      ...
      try:
          testLetStatement(s, tests[i].expectedIdentifier)
      ...
      # try:
      #   testLiteralExpression(val, expectedValues[i])
      # except ValueError as e:
      #   echo e.msg
      #   check(false)


#proc testLiteralExpression(exp: ast.Expression, )
